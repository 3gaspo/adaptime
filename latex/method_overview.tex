...
Each task and range declares an alignment period, fixed and rolling strides,
and a bounded retrieval lookback in the dataset configuration. Adaptation
training and validation retain only rows with this complete retrieval history,
so distances compare equal-width representations. Foundation forecasts remain
separate: they use all history available at an origin, capped at the backbone
limit (8192 for Chronos-2, 4096 for TS-ICL, and 2048 for Chronos-Bolt).
TS-RAG retains its separate native length of 512 when used as an external
control. Every official test window remains present.
...
Validation first averages each candidate's MSSE across variates at every
date. A paired moving-date-block bootstrap estimates the standard error of
each candidate's loss difference from the observed best. The selected set is
all candidates within one standard error; its deterministic preference is
vanilla, then lower $K$, then larger $\alpha$. The automatic block length is
$\max(\operatorname{round}(T^{1/3}),\lceil H/s_f\rceil)$ for $T$ validation
dates, capped below $T$; 1000 replications are used by default.
...
selected shared Seasonal artifact fixes the evaluation grid to cells with
finite target support, finite Seasonal predictions on that support, and finite
Seasonal MASE. Foundation forecasts and vanilla must be finite wherever that
grid expects a prediction. If a fixed Adaptime candidate, rolling Ridge, or
TS-RAG is non-finite there, its complete cell forecast is replaced by vanilla;
the fallback mask and count are retained. Comparisons require identical grid
metadata and report finite, grid, and total counts for each metric.
...
cache identities are retained with the task artifacts. Only reusable
source-window backbone forecasts are shared across fixed and rolling methods,
through readable configuration-matched \texttt{run\_n} cache artifacts. Retrieval
representations are recomputed; canonical test vanilla and composite covariate
forecasts remain in their phase artifacts. Neighbor selections remain
method-specific. The resulting adapted estimands are transparent gated hybrids:
