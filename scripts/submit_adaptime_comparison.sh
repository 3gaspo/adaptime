...
vanilla_job="$(sbatch --parsable --job-name=adaptime_vanilla --dependency="afterok:$prepare_job" "--export=ALL,TIME_LAUNCH_ID=$vanilla_launch_id,ADAPTIME_METHOD=vanilla,ADAPTIME_STAGE=vanilla" "$family_front")"
...
printf 'workflow=%s prepare=%s vanilla=%s ridge=%s tsrag=%s report=%s seasonal=shared\n' \
    "$workflow_id" "$prepare_job" "$vanilla_job" "$ridge_job" "$tsrag_job" "$report_job"
