#!/bin/bash
# TimesFM-3 experiments for all TIME benchmark datasets
# Generated for TimesFM-3 evaluation

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
ENV_NAME="${ENV_NAME:-timesfm3}"
...
setup_timesfm_repo() {
    if [ ! -d "$TIMESFM_DIR/.git" ]; then
        log_info "Cloning TimesFM-3 repo..."
...
setup_conda_env() {
    ...
    if conda env list | awk '{print $1}' | grep -x "$ENV_NAME" >/dev/null 2>&1; then
        ...
        cd "$TIMESFM_DIR"
        pip install -e .
        pip install datasets gluonts dotenv torch huggingface_hub
        cd "$ROOT_DIR"
    fi
}
